...
PUBLICATION_IMAGE_SCHEMA = "cybermancy-step4-class-publication-images-v1.1"
    ...
    for entity in class_entities:
        ...
        if mapping_error is not None or logical_rel is None:
            if mapping_error == "missing":
                ...
                message = "Class/Subclass Foundry image reference did not map to a logical repository asset path."
            errors.append(
                _issue(
                    entity,
        ...
        if resolution_status != "resolved":
            if resolution_status == "ambiguous":
                code = "CLASS_PUBLICATION_IMAGE_SOURCE_AMBIGUOUS"
                message = (
                    "Class/Subclass logical image path resolves to multiple checked-in assets "
                    "with different contents; Step 4 will not choose one implicitly."
                )
            else:
                code = "CLASS_PUBLICATION_IMAGE_MISSING"
                message = (
                    "Class/Subclass publication image was not found at the logical repository path "
                    "or its audience-specific docs asset location."
                )
            errors.append(
                _issue(
                    entity,
        ...
        if staged.get("status") != "staged":
            errors.append(
                _issue(
                    ...
                    "Class/Subclass publication image disappeared from the resolved repository path before staging.",
                    sourceReference=raw_image,
                    logicalReference=logical_rel,
                    sourceRepoPath=source_repo_rel,
                    publicationPath=publication_rel,
    ...
    if errors:
        ...
        add_check(
            ...
            (
                ...
                "Class/Subclass entities; checked-in artwork may remain under audience-specific docs roots."
            ),
            summary,
        )
    _write_json(validation_path, report)
