...
def manifest_contract_report(pub: dict, asm: dict, config: dict) -> dict:
    ...
    if declared_algorithms=={STRUCTURED_DIGEST_ALGORITHM} and configured_algorithm==STRUCTURED_DIGEST_ALGORITHM:
        add_check(report,'STRUCTURED_DIGEST_CONTRACT','PASS','Publication manifest and normalization config use the shared structured-family digest v4 contract.')
    else:
...
def repository_preflight(repo_root: Path, pub: dict, asm: dict, config: dict, report: dict):
    ...
    # Structured canonical sources: existence + shared digest-v4 + entity counts.
    missing_fams=[]
    structured_errors=[]
    ...
    if structured_errors:
        ...
        if digest_details:
            item=add_check(
                ...
                'One or more structured families do not match the frozen digest-v4 snapshot.',
                digest_details
            ); source_errors.append(item)
        if count_details:
            item=add_check(
        ...
        add_check(
            ...
            'match the frozen digest-v4/count snapshot.',
            {
