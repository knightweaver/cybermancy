...
def _feature_tex(feature: dict[str, Any], *, compact: bool = False) -> str:
    ...
    if compact:
        lines = [
            r"\Needspace{0.42in}",
            rf"{{\fontsize{{9}}{{10.2}}\selectfont\bfseries\color{{CMInk}} {name}\par}}",
            r"{\color{CMBright}\rule{\linewidth}{0.35pt}}",
        ]
        if description:
            lines.append(rf"{{\fontsize{{8}}{{9.5}}\selectfont {description}\par}}")
        else:
            lines.append(r"{\fontsize{7.6}{9}\selectfont\itshape\color{CMMuted} No publication description supplied.\par}")
        lines.append(r"\vspace{0.8mm}")
        return "\n".join(lines)
...
def _class_opening_tex(view: dict[str, Any], config: dict[str, Any], source_root: Path, output_dir: Path) -> str:
    ...
    pieces = [
        ...
        r"\rowcolor{CMSoft}",
        r"{\fontsize{8}{9}\selectfont\bfseries\color{CMMuted} HIT POINTS} & {\fontsize{8}{9}\selectfont\bfseries\color{CMMuted} EVASION} \\",
...
def _class_support_tex(cls: dict[str, Any]) -> str:
    ...
    if inventory or guide or class_items:
        ...
        pieces.extend([r"\end{tabularx}", r"\vspace{1mm}"])
...
def _subclass_tex(
    ...
    pieces = [
        r"\vspace{0pt}",
        r"{\fontsize{6.7}{7.5}\selectfont\bfseries\color{CMBright} SUBCLASS\par}",
    ...
    if trait:
        pieces.extend([
            ...
            rf"\fontsize{{7.5}}{{8.5}}\selectfont\bfseries\color{{CMInk}} SPELLCAST TRAIT: {latex_escape(trait.upper())}",
            r"}}",
            r"\vspace{0.8mm}",
        ])
    if description:
        pieces.append(rf"{{\fontsize{{7.7}}{{9.2}}\selectfont {description}\par}}")
    else:
        pieces.append(r"{\fontsize{7.4}{8.8}\selectfont\itshape\color{CMMuted} No subclass lead text is currently supplied by Step 4.\par}")
    pieces.extend([r"\end{minipage}", r"\vspace{1mm}"])
    ...
    for stage in (config.get("composition") or {}).get(
        ...
        pieces.extend([
            r"\Needspace{0.38in}",
            rf"{{\fontsize{{11.5}}{{12.5}}\selectfont\bfseries\color{{CMAccent}} {latex_escape(stage_name.upper())}\par}}",
            r"\vspace{0.3mm}",
        ])
        if rows:
            pieces.extend(_feature_tex(row, compact=True) for row in rows)
        else:
            pieces.append(r"{\fontsize{7.4}{8.8}\selectfont\itshape\color{CMMuted} No features at this progression stage.\par}")
    return "\n".join(pieces)
