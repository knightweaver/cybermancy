...
def _style(config: dict[str, Any]) -> dict[str, float | str]:
    ...
    return {
        "accent": str(source.get("accentColor", "0B6573")), "bright": str(source.get("accentBrightColor", "18A7B5")), "ink": str(source.get("inkColor", "183238")), "muted": str(source.get("mutedColor", "58747A")), "soft": str(source.get("softColor", "EAF4F5")), "rule": str(source.get("ruleColor", "B9D9DC")),
        "margin": float(source.get("pageMarginIn", 0.55) or 0.55), "column_sep": float(source.get("columnSepIn", 0.24) or 0.24),
        "body": body, "body_leading": body_leading, "title": title, "title_leading": title_leading, "type": entry_type, "type_leading": entry_type_leading,
        "action": action_title, "action_leading": action_leading, "meta": meta, "meta_leading": meta_leading,
        "entry_needspace": float(source.get("entryMinStartSpaceIn", 0.72) or 0.72), "action_needspace": float(source.get("actionMinStartSpaceIn", 0.42) or 0.42),
    }
...
def _format_damage(value: Any) -> str:
    ...
    for part in parts:
        if not isinstance(part, dict):
            continue
        target = str(part.get("applyTo") or "").strip()
        types = part.get("types")
        type_text = "/".join(str(x) for x in types if str(x)) if isinstance(types, list) else str(types or "").strip()
...
def _metadata_lines(action: dict[str, Any]) -> list[tuple[str, str]]:
    ...
    if cost: rows.append(("Cost", cost))
    if action.get("range") not in (None, ""): rows.append(("Range", str(action.get("range"))))
    target = _format_target(action.get("target"))
    if target: rows.append(("Target", target))
    damage = _format_damage(action.get("damage"))
    if damage: rows.append(("Damage", damage))
    uses = _format_uses(action.get("uses"))
    if uses: rows.append(("Uses", uses))
    return rows
...
def _action_tex(action: dict[str, Any], style: dict[str, Any]) -> str:
    ...
    pieces = [
        ...
        rf"{{\fontsize{{{style['action']:.2f}}}{{{style['action_leading']:.2f}}}\selectfont\bfseries\color{{CMInk}} {_inline_tex(name)}" + (rf"{{\color{{CMMuted}}\mdseries {_inline_tex(suffix)}}}" if suffix else "") + r"\par}",
    ]
    rules = markdown_to_tex(action.get("rulesMarkdown"))
    if rules:
        pieces.extend([r"\begingroup", r"\setlength{\parskip}{0.6mm}", r"\setlength{\parindent}{0pt}", rf"\fontsize{{{style['body']:.2f}}}{{{style['body_leading']:.2f}}}\selectfont\color{{CMInk}}", rules, r"\endgroup"])
    for label, value in _metadata_lines(action):
        pieces.append(rf"{{\fontsize{{{style['meta']:.2f}}}{{{style['meta_leading']:.2f}}}\selectfont\color{{CMMuted}}\textbf{{{latex_escape(label)}:}} {_inline_tex(value)}\par}}")
    return "\n".join(pieces)
...
def _resource_tex(resource: Any, style: dict[str, Any]) -> str:
    if not isinstance(resource, dict) or not resource:
        return ""
    pieces: list[str] = []
    if resource.get("type") not in (None, ""): pieces.append(str(resource.get("type")))
    if resource.get("value") not in (None, ""): pieces.append(f"value {resource.get('value')}")
    if resource.get("max") not in (None, ""): pieces.append(f"max {resource.get('max')}")
    if not pieces:
        return ""
    return rf"{{\fontsize{{{style['meta']:.2f}}}{{{style['meta_leading']:.2f}}}\selectfont\color{{CMMuted}}\textbf{{Resource:}} {_inline_tex('; '.join(pieces))}\par}}"
...
def _entry_tex(entry: dict[str, Any], config: dict[str, Any]) -> str:
    ...
    pieces = [
        ...
        rf"{{\fontsize{{{style['title']:.2f}}}{{{style['title_leading']:.2f}}}\selectfont\bfseries\color{{CMInk}} {_inline_tex(name)}\par}}",
        rf"{{\fontsize{{{style['type']:.2f}}}{{{style['type_leading']:.2f}}}\selectfont\bfseries\color{{CMAccent}} {type_label}\par}}",
        r"\vspace{0.8mm}",
    ]
    rules = markdown_to_tex(entry.get("rulesMarkdown"))
    if rules:
        pieces.extend([r"\begingroup", r"\setlength{\parskip}{0.8mm}", r"\setlength{\parindent}{0pt}", r"\setlength{\emergencystretch}{1.5em}", rf"\fontsize{{{style['body']:.2f}}}{{{style['body_leading']:.2f}}}\selectfont\color{{CMInk}}", rules, r"\endgroup"])
    actions = entry.get("actions")
    if isinstance(actions, list) and actions:
        pieces.extend([r"\vspace{1.0mm}", rf"{{\fontsize{{{style['meta']:.2f}}}{{{style['meta_leading']:.2f}}}\selectfont\bfseries\color{{CMAccent}} ACTIONS\par}}"])
    ...
    pieces.extend([r"\vspace{1.5mm}", r"{\color{CMRule}\rule{\linewidth}{0.35pt}}", r"\vspace{2.0mm}"])
    return "\n".join(pieces)
...
def _chapter_header(view: dict[str, Any], config: dict[str, Any]) -> str:
    ...
    pieces = [rf"{{\fontsize{{7.6}}{{8.6}}\selectfont\bfseries\color{{CMBright}} CHAPTER {chapter} / {part_label}\par}}", r"\vspace{1.6mm}", rf"{{\fontsize{{30}}{{31}}\selectfont\bfseries\color{{CMInk}} {title.upper()}\par}}", r"\vspace{1.2mm}", r"{\color{CMBright}\rule{\linewidth}{0.9pt}}"]
    if intro:
        pieces.extend([r"\vspace{2.0mm}", r"\begingroup", r"\setlength{\parskip}{0pt}", r"\setlength{\parindent}{0pt}", r"\fontsize{10.5}{12.6}\selectfont\color{CMInk}", intro, r"\endgroup"])
    pieces.append(r"\vspace{3.0mm}")
    return "\n".join(pieces)
...
def _group_tex(group: dict[str, Any], config: dict[str, Any]) -> str:
    ...
    pieces = [r"\Needspace{0.9in}", r"\noindent\begin{minipage}{\linewidth}", rf"{{\fontsize{{18}}{{19}}\selectfont\bfseries\color{{CMAccent}} {title}\hfill\fontsize{{7.6}}{{8.6}}\selectfont\bfseries\color{{CMMuted}} {len(entries)} {count_label}\par}}", r"\vspace{0.8mm}", r"{\color{CMRule}\rule{\linewidth}{0.55pt}}", r"\end{minipage}", r"\vspace{1.8mm}", rf"\begin{{multicols}}{{{columns}}}", r"\raggedcolumns", rf"\setlength{{\columnsep}}{{{style['column_sep']:.3f}in}}"]
...
def render_ice_reference_tex(view: dict[str, Any], config: dict[str, Any]) -> str:
    ...
    return "\n".join([
        ...
        rf"\usepackage[letterpaper,margin={style['margin']:.3f}in]{{geometry}}",
        r"\usepackage{fontspec}", r"\usepackage{xcolor}", r"\usepackage{multicol}", r"\usepackage{needspace}",
        r"\setlength{\parindent}{0pt}", r"\setlength{\parskip}{0pt}", r"\pagestyle{plain}",
        rf"\definecolor{{CMAccent}}{{HTML}}{{{style['accent']}}}", rf"\definecolor{{CMBright}}{{HTML}}{{{style['bright']}}}", rf"\definecolor{{CMInk}}{{HTML}}{{{style['ink']}}}", rf"\definecolor{{CMMuted}}{{HTML}}{{{style['muted']}}}", rf"\definecolor{{CMSoft}}{{HTML}}{{{style['soft']}}}", rf"\definecolor{{CMRule}}{{HTML}}{{{style['rule']}}}",
        r"\begin{document}", r"\raggedbottom", *body, r"\end{document}", "",
    ])
