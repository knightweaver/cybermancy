...
def resolve_equipment_section(section_registry: Path, config_dir: Path) -> tuple[list[dict], list[dict]]:
    ...
    for entry in entries:
        ...
        item = {"family": family, "chapter": chapter, "title": title, "config": str(config_path), "configPresent": config_path.is_file()}
        if config_path.is_file():
            try:
                config = _load_json(config_path)
                if str(config.get("family") or "") != family or int(config.get("chapter") or 0) != chapter:
                    errors.append({"issue": "section-config-mismatch", "family": family, "chapter": chapter, "config": str(config_path), "configFamily": config.get("family"), "configChapter": config.get("chapter")})
            except Exception as exc:
...
def _aggregate_shell(operation: str, config_dir: Path, section_registry: Path, sidecar: Path, manuscript: Path) -> dict:
    return {"schema": AGGREGATE_SCHEMA, "operation": operation, "status": "PASS", "checks": [], "warnings": [], "errors": [], "inputs": {"configDirectory": str(config_dir), "sectionRegistry": str(section_registry), "structuredEntities": str(sidecar), "assembledManuscript": str(manuscript)}, "families": []}
...
def _child_command(operation: str, item: dict, args: argparse.Namespace, *, script_path: Path, output_base: Path | None) -> list[str]:
    ...
    if operation == "build":
        if output_base is not None: command.extend(["--output-dir", str(output_base / f"chapter{int(item['chapter'])}")])
        if getattr(args, "tex_only", False): command.append("--tex-only")
    return command
...
def _invoke_child(command: list[str]) -> tuple[int, dict]:
    ...
    env["PYTHONUTF8"] = "1"
    proc = subprocess.run(command, text=True, encoding="utf-8", errors="replace", stdout=subprocess.PIPE, stderr=subprocess.PIPE, env=env)
    stdout = (proc.stdout or "").strip()
    try:
        payload = json.loads(stdout) if stdout else {}
    except json.JSONDecodeError:
        payload = {"status": "FAIL", "errors": [{"code": "CHILD_OUTPUT_JSON", "status": "ERROR", "message": "Equipment child command did not return valid JSON.", "details": {"stdout": stdout[-12000:], "stderr": (proc.stderr or "")[-12000:]}}]}
    if proc.returncode != 0 and payload.get("status") != "FAIL":
        payload["status"] = "FAIL"
        payload.setdefault("errors", []).append({"code": "CHILD_COMMAND_EXIT", "status": "ERROR", "message": f"Equipment child command exited with status {proc.returncode}.", "details": (proc.stderr or "")[-12000:]})
...
def _family_summary(item: dict, payload: dict, returncode: int, report_dir: Path, *, stage: str) -> dict:
    ...
    return {"chapter": item["chapter"], "family": item["family"], "title": item["title"], "config": item["config"], "stage": stage, "status": status, "returnCode": returncode, "reportPath": str(_family_report_path(report_dir, item["family"])), "report": payload}


def _missing_summary(item: dict, *, stage: str) -> dict:
    return {"chapter": item["chapter"], "family": item["family"], "title": item["title"], "config": item["config"], "stage": stage, "status": "BLOCKED", "returnCode": None, "reportPath": None, "report": None, "reason": "Required Equipment family config has not yet been implemented."}
...
def _section_checks(report: dict, section: list[dict], errors: list[dict]) -> bool:
    if errors:
        ...
        return False
    _add_check(report, "EQUIPMENT_SECTION_CONTRACT", "PASS", f"Resolved {len(section)} required Equipment & Technology chapter(s) from the section registry.", [{"chapter": x["chapter"], "family": x["family"], "config": x["config"]} for x in section])
    missing = [x for x in section if not x["configPresent"]]
    _add_check(report, "EQUIPMENT_CONFIG_COVERAGE", "BLOCKED" if missing else "PASS", f"{len(missing)} required Equipment family config(s) are not yet implemented." if missing else "Every required Equipment family has an approved Step 6 config.", [{"chapter": x["chapter"], "family": x["family"], "config": x["config"]} for x in missing] or None)
...
def _run_phase(operation: str, implemented: list[dict], args: argparse.Namespace, *, script_path: Path, report_dir: Path, output_base: Path | None, stop_on_failure: bool) -> tuple[list[dict], bool]:
    ...
    for item in implemented:
        if failed and stop_on_failure:
            summaries.append({"chapter": item["chapter"], "family": item["family"], "title": item["title"], "config": item["config"], "stage": operation, "status": "BLOCKED", "returnCode": None, "reportPath": str(_family_report_path(report_dir, item["family"])), "report": None, "reason": "A prior implemented Equipment family failed during this batch phase."})
            continue
        returncode, payload = _invoke_child(_child_command(operation, item, args, script_path=script_path, output_base=output_base))
...
def run_all_equipment_command(operation: str, args: argparse.Namespace, *, script_path: Path, config_dir: Path, default_sidecar: Path, default_manuscript: Path, default_report_dir: Path, section_registry: Path | None = None) -> int:
    ...
    """
    if operation not in {"inspect", "validate", "build"}: raise ValueError(f"Unsupported Equipment batch operation: {operation}")
    section_registry = (section_registry or (config_dir / SECTION_FILENAME)).resolve()
    ...
    aggregate = _aggregate_shell(operation, config_dir.resolve(), section_registry, sidecar, manuscript)
    section, section_errors = resolve_equipment_section(section_registry, config_dir.resolve())
    if not _section_checks(aggregate, section, section_errors):
        if operation != "inspect": _write_json(report_dir / "equipment-all.json", aggregate)
        print(json.dumps(aggregate, indent=2, ensure_ascii=False)); return 2
    ...
    if operation == "inspect":
        ...
        print(json.dumps(aggregate, indent=2, ensure_ascii=False)); return 0 if aggregate["status"] == "PASS" else 2
    if operation == "validate":
        ...
        _write_json(report_dir / "equipment-all.json", aggregate); print(json.dumps(aggregate, indent=2, ensure_ascii=False)); return 0 if aggregate["status"] == "PASS" else 2
    ...
    if preflight_failed:
        ...
        _write_json(report_dir / "equipment-all.json", aggregate); print(json.dumps(aggregate, indent=2, ensure_ascii=False)); return 2
    ...
    _add_check(aggregate, "EQUIPMENT_FAMILY_RESULTS", "ERROR" if build_failed else "PASS", "An implemented Equipment family build failed; later implemented builds were blocked." if build_failed else "All implemented Equipment families built successfully.")
    _write_json(report_dir / "equipment-all.json", aggregate); print(json.dumps(aggregate, indent=2, ensure_ascii=False)); return 0 if aggregate["status"] == "PASS" else 2
