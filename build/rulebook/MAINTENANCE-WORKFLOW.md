...
## 8. Structured corpus cardinality authority

For mutable structured publication content, the selected, reviewed, committed publication manifest is the canonical expected cardinality authority. A refreshed manifest therefore authorizes an intentional corpus-size change only after the canonical source change and refreshed inventory/manifest/configuration artifacts have been reviewed and committed. Frozen Step 6 layout acceptance counts are historical regression provenance; they do not define the current runtime corpus size.

The direct manifest-backed families are `classes`, `subclasses`, `domains`, `weapons`, `ammo`, `armors`, `cybernetics`, `drones-devices`, `consumables`, `mods`, `loot`, `adversaries`, and `environments`. Production also validates the source families `features` and `adversaries-features` because they feed specialized projections. Their authority chain is:

```text
selected publication manifest expected corpus
-> Step 4 normalized semantic corpus and semantic IDs
-> Stage 20-120 family/projection lowering
-> Stage 130 publication-shell readiness and structured family coverage
-> Stages 140-170 rendered and publication regression
```

Any manifest/Step 4 disagreement is a blocking error. Stage 130 must not introduce an independent mutable count; it accepts the already reconciled Step 4 corpus and requires every applicable structured family body to survive into the publication shell exactly once. The later validation stages then require that lowered package content survives to the rendered output. Adding a valid Domain card, weapon, ammunition item, loot item, Class, Subclass, Adversary, or Environment therefore requires refreshing the normal inventory/manifests and rebuilding, not editing a renderer count.

### Specialized projections

Chapter 29 ICE is a projection of the manifest-authoritative `features` source family. The publication set is selected by Step 4 `iceSemantics.semanticIds`; sentry/wall counts are derived from that selection. A new valid ICE entry requires the refreshed `features` manifest authority and Step 4 ICE semantics, not a Chapter 29 fixed-count edit.

Chapter 32 Adversary Features is a projection of the manifest-authoritative `adversaries-features` source family. Step 4's approved publication-equivalence selection (`representativeSemanticIds`) determines the published representatives and must partition the authoritative source family with the excluded IDs. A new valid Adversary Feature is published according to those Step 4 equivalence semantics; it does not require a manual Chapter 32 count edit.

Historical acceptance counts may remain in frozen layout contracts or regression fixtures only when explicitly non-operative. They are useful provenance for detecting accidental design drift, but they are not current corpus authority.

### Character Origins boundary

Character Origins remain intentionally outside generalized structured-family count authority. Chapters 10-11 are prose-derived editorial entries governed by the accepted Character Option Entry grammar. Their exact identity, order, feature-count, and artwork checks remain blocking and must not be weakened by structured-authority maintenance. The currently frozen editorial expectations of 18 Ancestories, 9 Communities, and 27 staged artworks are structural acceptance invariants for this prose-derived contract, not manifest-backed mutable corpus counts.

If Character Origins are later converted to a manifest-driven structured corpus, that requires a separate design/migration decision. Do not infer that migration from this authority system.

### Diagnostic surfaces

Production preflight writes `build/rulebook/reports/preflight.json`. Its `structuredCorpusAuthority` and `structuredProjectionAuthority` sections expose the family/projection name, authority descriptor, manifest expected count, Step 4 actual count, semantic-ID coverage, projection subtype/group distributions where applicable, reconciliation status, and actionable errors. Successful profile build reports under `build/rulebook/reports/<profile>/build-report.json` retain those authority reports, record the post-pipeline `structuredEntityCounts`, link every Stage 130-170 report, and record the final page count/output path. Stage reports remain the rendered-coverage evidence; they must be reviewed together with the top-level build report when a corpus changes.

Reader-facing PDFs must not expose internal absolute filesystem paths. Build/report paths are diagnostic metadata only.

### Final validation commands

From a clean Windows checkout after canonical content and refreshed freeze artifacts are committed:

```powershell
python build\rulebook\scripts\build-rulebook.py baseline-check --verbose
python build\rulebook\scripts\maintain-rulebook.py status --verbose
python build\rulebook\scripts\build-rulebook-source.py --verbose validate
...
For a release checkpoint after both profile builds pass:

```powershell
python build\rulebook\scripts\maintain-rulebook.py release --profile all
```

A production acceptance requires both profiles to complete Stages 130-170, successful rendered regression, and openable final PDFs. Read-only CI success is necessary but is not a substitute for the proven local Pandoc/LuaLaTeX/MiKTeX production render.