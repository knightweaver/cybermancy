...
7. Each Subclass uses compact artwork floated at the left of its lead with `wrapfigure`. Spellcast Trait and lead description flow beside the artwork and continue beneath it when the lead is long enough. Missing artwork omits the float without changing the remaining identity/lead grammar.
8. The Subclass lead wrap is explicitly terminated with `\WFclear` before Foundation. If the lead is shorter than the artwork, the renderer first reserves the remaining `wrapfig` line depth so progression content begins below rather than on top of the image. Foundation, Specialization, and Mastery therefore always return to the full width of the Subclass `paracol` column and preserve variable-length progression arrays. Feature descriptions may break naturally across pages while headings retain enough following space to avoid obvious orphans.
...
12. LuaLaTeX rendering requires both `paracol` for page-breakable parallel Subclass columns and `wrapfig` for Subclass lead artwork wrapping. The builder preflights TeX dependencies when `kpsewhich` or `findtexmf` is available and emits an explicit installation diagnostic if a required package is missing.
