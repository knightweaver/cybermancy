...
def collect_asset_refs(doc: dict) -> list[str]:
    refs=[]
    def walk(v: Any, key: str=''):
        ...
        elif isinstance(v,str) and key in {'img','src','texture'}:
            if v and not v.startswith(('icons/','http://','https://')):
                refs.append(v)
    walk(doc)
...
def render_entity(family: str, doc: dict, fast_play_paths: list[str]) -> tuple[str, dict]:
    ...
    metadata={
        ...
        'fastPlay':fast_meta,'assetRefs':collect_asset_refs(doc)
    }
    return '\n'.join(lines).rstrip()+'\n',metadata
