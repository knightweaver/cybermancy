<div class="domain" markdown="1">
<img src="../../assets/icons/domains/expert-driver.webp" alt="Expert Driver" class="item-image">

# Expert Driver Domain
<div class="item-subtitle"></div>

## Tenets


## Domain Moves / Boons


---

## Description
&lt;i&gt;Speed racer's got nothing on you!&lt;/i&gt;&lt;p class="Card-Feature"&gt;All Driving checks are +2.  Each time you are Spotlighted while Driving, you may also take an Attack Action.&lt;/p&gt;

<div class="meta" markdown="1">
**Source:** *(fill in)* • **UUID:** `Compendium.cybermancy.system.expert-driver`
</div>
</div>
