<div class="adversary" markdown="1">
# Laser Tripwire Grid
<img src="../../../assets/icons/adversaries/laser-tripwire-grid.webp" alt="Laser Tripwire Grid" class="item-image" style="width:300px; height:auto;">

<div class="item-flavor" markdown="1">
A fixed-beam laser intrusion grid that activates lethal countermeasures when crossed.
</div>

<table class="stat-table">
  <thead><tr><th>Attribute</th><th align="right">Value</th></tr></thead>
  <tbody>
    <tr><td>Tier</td><td align="right">1</td></tr>
    <tr><td>Role</td><td align="right">Support</td></tr>
    <tr><td>Difficulty</td><td align="right">10</td></tr>
    <tr><td>Damage Thresholds</td><td align="right">3 / 5</td></tr>
    <tr><td>Hit Points</td><td align="right">3</td></tr>
    <tr><td>Stress</td><td align="right">1</td></tr>
  </tbody>
</table>

## Attack

| Attack | Modifier | Range | Damage |
|---|---:|---|---|
| Laser Surge | +2 | Melee | d6 Physical |

## Experiences
- **Loot the grid controller +1**

## Motives & Tactics
Detect movement and trigger ambush responses; holds area denial patterns.

## Features
—




---

<div class="meta" markdown="1">
adversaries/Security devices/Support/Tier 1
</div>
</div>
